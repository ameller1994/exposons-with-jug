...
def featurize(
        tag, trajectories, topology, probe_radius=2.8, **kwargs):
    """Run the whole pockets pipeline
    """

    probe_radius = '%.2f' % probe_radius
    DATA_STEM = util.bowmanlab_data_stem(tag, probe_radius)
    ...
    sidechain_sasas = [tasks.condense_sparse_sidechain_sasas(sasa, topology)
                       for sasa in sasas]
    if 'mask' in kwargs:
        sidechain_sasas = np.array(sidechain_sasas)[kwargs['mask']]
...
def cluster(
        tag, trajectories, topology, sasa_sidechain_h5,
        cluster_radii=[], cluster_numbers=[],
        cluster_algorithm='khybrid', cluster_distance='euclidean',
        kmedoids_updates=5, lag_times='10:250:12', timestep=25, stride=1):
    ...
    for radius in cluster_radii:
        ...
        CLUSTER_STEM = sc_sasa_filename \
            ...
            .replace('/features/', '/cluster/') + \
            "-".join(['sidechains', cluster_algorithm, str(radius),
                      cluster_distance] + (
                          ['kmedoids' + str(kmedoids_updates)]
                          if cluster_algorithm == 'khybrid'
                          else []))
    ...
    for k in cluster_numbers:
        ...
        CLUSTER_STEM = sc_sasa_filename \
            .replace('sasas-sidechains.h5', '') \
            .replace('/features/', '/cluster/') + \
            "-".join(['sidechains', cluster_algorithm, k,
                      cluster_distance] + (
                          ['kmedoids' + str(kmedoids_updates)]
                          if cluster_algorithm == 'khybrid'
                          else []))
